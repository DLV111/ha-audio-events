...
def test_audio_fixture_classifies_as_expected(
    fixture_name: str, expected_label: str, category: str
) -> None:
    ...
    if not model_path.exists() or not (
        labels_path.exists() or legacy_labels_path.exists()
    ):
        pytest.skip(
            "YAMNet model artifacts are not present; install them to run classifier integration tests"
        )
    ...
    matching = [
        r
        for r in results
        if expected_label.lower() in r.lower()
        or category.lower() in r.lower()
        or "rail" in r.lower()
        or "bark" in r.lower()
    ]
